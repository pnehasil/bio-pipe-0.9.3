ADDVKS(OUTDIR,RUN,DBS)
   ...
   ;
   ; 27.06.2021
   ; clinvar_sig i clinvar_revstat z gemini, potazmo z globalu ^WTMP
   ...
   . . S PR=$P(^WTMP(PAC,R),"|",1,8)
   . . S DR=$P(^WTMP(PAC,R),"|",9,11)
   . . S TR=$P(^WTMP(PAC,R),"|",12,39)
   . . S CT=$P(^WTMP(PAC,R),"|",40,47)
   ...
   F  S PAC=$O(^WTMPU(PAC)) Q:PAC=""  D
   . S FPAC=$P(PAC,"_",2)
   . S OFILE=OUTDIR_"/"_PAC_".sql" 
   . OPEN OFILE:NEWVERSION
   . S R=0
   . USE OFILE
   . D CRTABLE(FPAC,RUN,DBS)
   . F  S R=$O(^WTMPU(PAC,R)) Q:R=""  D
   . . S gene=$P(^WTMPU(PAC,R),"|",1)
   . . S codonXchange=$P(^WTMPU(PAC,R),"|",2)
   . . S aaXchange=$P(^WTMPU(PAC,R),"|",3)
   . . S xexon=$P(^WTMPU(PAC,R),"|",4)
   . . S exon=$$XEXON(gene,xexon)
   . . S qual=$P(^WTMPU(PAC,R),"|",5)
   . . S depth=$P(^WTMPU(PAC,R),"|",6)
   . . S numXalleles=$P(^WTMPU(PAC,R),"|",7)
   . . S clinvarXsig=$P(^WTMPU(PAC,R),"|",8)
   . . S x01XUBEOXClass=$P(^WTMPU(PAC,R),"|",9)
   . . S CzecancaXkons=$P(^WTMPU(PAC,R),"|",10)
   . . S maxXaafXall=$P(^WTMPU(PAC,R),"|",11)
   . . S aafXespXea=$P(^WTMPU(PAC,R),"|",12)
   . . S impactXso=$P(^WTMPU(PAC,R),"|",13)
   . . S VIKX9carrier=$P(^WTMPU(PAC,R),"|",14)
   . . S VIKXunknown=$P(^WTMPU(PAC,R),"|",15)
   . . S aafXespXall=$P(^WTMPU(PAC,R),"|",16)
   . . S aafX1kgXeur=$P(^WTMPU(PAC,R),"|",17)
   . . S aafX1kgXall=$P(^WTMPU(PAC,R),"|",18)
   . . S gmsXillumina=$P(^WTMPU(PAC,R),"|",19)
   . . S gmsXsolid=$P(^WTMPU(PAC,R),"|",20)
   . . S gmsXiontorrent=$P(^WTMPU(PAC,R),"|",21)
   . . S cosmicXids=$P(^WTMPU(PAC,R),"|",22)
   . . S caddXraw=$P(^WTMPU(PAC,R),"|",23)
   . . S caddXscaled=$P(^WTMPU(PAC,R),"|",24)
   . . S aafXexacXall=$P(^WTMPU(PAC,R),"|",25)
   . . S aafXadjXexacXall=$P(^WTMPU(PAC,R),"|",26)
   . . S aafXadjXexacXnfe=$P(^WTMPU(PAC,R),"|",27)
   . . S exacXnumXhet=$P(^WTMPU(PAC,R),"|",28)
   . . S exacXnumXhomXalt=$P(^WTMPU(PAC,R),"|",29)
   . . S aaXlength=$P(^WTMPU(PAC,R),"|",30)
   . . S transcript=$P(^WTMPU(PAC,R),"|",31)
   . . S chrom=$P(^WTMPU(PAC,R),"|",32)
   . . S start=$P(^WTMPU(PAC,R),"|",33)
   . . S end=$P(^WTMPU(PAC,R),"|",34)
   . . S ref=$P(^WTMPU(PAC,R),"|",35)
   . . S alt=$P(^WTMPU(PAC,R),"|",36)
   . . S numXreadsXwXdels=$P(^WTMPU(PAC,R),"|",37)
   . . S alleleXcount=$P(^WTMPU(PAC,R),"|",38)
   . . S alleleXbal=$P(^WTMPU(PAC,R),"|",39)
   . . S rsXids=$P(^WTMPU(PAC,R),"|",40)
   . . S inXomim=$P(^WTMPU(PAC,R),"|",41)
   . . S clinvarXdiseaseXname=$P(^WTMPU(PAC,R),"|",42)
   . . S rmsk=$P(^WTMPU(PAC,R),"|",43)
   . . S zwt=$P(^WTMPU(PAC,R),"|",44)
   . . S zalt=$P(^WTMPU(PAC,R),"|",45)
   . . S gunk=$P(^WTMPU(PAC,R),"|",46)
   . . S prio=$$PRIORITY(gene)
   . . S group=$$GROUP(gene)
   . . S aafXgnomadXall=$P(^WTMPU(PAC,R),"|",47)
   . . S aafXgnomadXnfe=$P(^WTMPU(PAC,R),"|",48)
   . . S aafXgnomadXnonXcancer=$P(^WTMPU(PAC,R),"|",49)
   . . S gnomadXpopmaxXAF=$P(^WTMPU(PAC,R),"|",50)
   . . S gnomadXnumXhet=$P(^WTMPU(PAC,R),"|",51)
   . . S gnomadXnumXhomXalt=$P(^WTMPU(PAC,R),"|",52)
   . . S gnomadXnumXchroms=$P(^WTMPU(PAC,R),"|",53)
   . . S clinvarXrev=$P(^WTMPU(PAC,R),"|",54)
   . . S pXp=$P(^WTMPU(PAC,R),"|",55)
   . . S pXs=$P(^WTMPU(PAC,R),"|",56)
   . . S sXp=$P(^WTMPU(PAC,R),"|",57)
   . . S sXs=$P(^WTMPU(PAC,R),"|",58)
   . . S pcar=$P(^WTMPU(PAC,R),"|",59)
   . . S sct=$P(^WTMPU(PAC,R),"|",60)
...
CRTABLE(TABLE,RUN,DBS)
   ...
   W "      `aaf_gnomad_all` decimal(16,14) DEFAULT NULL,",!
   W "      `aaf_gnomad_nfe` decimal(16,14) DEFAULT NULL,",!
   W "      `aaf_gnomad_non_cancer` decimal(16,14) DEFAULT NULL,",!
   W "      `gnomad_popmax_AF` decimal(16,14) DEFAULT NULL,",!
