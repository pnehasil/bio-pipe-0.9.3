...
FLOG=${WOR_LOGDIR}/${phase}_082_$(date +%y%m%d-%H%M%S).log
...
proc_recal() {
    ...
    ${JAVA8_BIN}java -Xmx${RECAL2_RAM} -Djava.io.tmpdir=${G_TMP_DIR} -jar ${GATK_DIR}/GenomeAnalysisTK.jar -T BaseRecalibrator \
    -R ${REF_GATK_DIR}/HG19uscs.fa \
    -L ${BED_DIR}/CZE1_2_APC_primary_targets.bed \
    -nct 4 \
    -ip 300 \
    -I $rfile \
    -knownSites ${DATABASE_GATK_DIR}/dbsnp_138.hg19_resorted.vcf \
    -knownSites ${DATABASE_GATK_DIR}/Mills_and_1000G_gold_standard.indels.hg19_resorted.vcf \
    -BQSR $ofile \
    -o $sfile >> ${FLOG} 2>&1
}
