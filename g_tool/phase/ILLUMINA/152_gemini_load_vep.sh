...
proc_gemini() {
   ...
   bf=`basename -s .n.final_norm.final_vep.vcf $ifile`
   dbfile=${GATK_VCF_JED_GEM}/$bf.GATK_regions_vep.db

   log "Start gemini load $ifile -> $dbfile"
   ${GEMINI_DIR}/gemini load -v $ifile -t all --cores 1 $dbfile >>${FLOG} 2>>${FLOG}
...
FLOG=${WOR_LOGDIR}/${phase}_152_$(date +%y%m%d-%H%M%S).log

#execute max n jobs at once
JOBMAX=${MAX_JOBS}
export JOBMAX
for f in `ls ${GATK_VCF_JED}/*.n.final_norm.final_vep.vcf`
  do 
